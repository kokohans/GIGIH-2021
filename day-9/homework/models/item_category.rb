...
class Item_category
    ...
    def self.get_categories_by_category_id(category_id)
        client = create_db_client
        qry_category = client.query("SELECT item_id FROM item_categories WHERE category_id=#{category_id}")
        if qry_category.count > 0
            return item_categories = Item_category.new(qry_category.first['item_id'], category_id)    
        else
            return nil
        end
    end
    ...
    def delete_category
        client = create_db_client
        qry_category = client.query("DELETE FROM item_categories WHERE category_id=#{@category_id}")
    end
